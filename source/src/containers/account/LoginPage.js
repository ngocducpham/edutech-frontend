...
import { cookieExists, setCookie } from "../../utils/cookieHelper";
      ...
      onCompleted: (responseData) => {
        if (responseData && responseData.token) {
          if (setUserData(responseData)) {
            ...
            else if (responseData.kind === UserTypes.STUDENT){
              this.redirectToAuthPage(getUserData());
            }else if (responseData.kind === UserTypes.TEACHER){
              this.redirectToAuthPage(getUserData());
            }

            const rootUrl = new URL(AppConstants.apiRootUrl);
            const parentDomain = rootUrl.hostname.split('.').slice(-2).join('.');
            const sevenDaysInSecond = 7 * 24 * 60 * 60;
            setCookie({
              name: 'token',
              value: responseData.token,
              domain: parentDomain,
              maxAge: sevenDaysInSecond,
              path: '/',
              sameSite: 'none',
              secure: true
            });
