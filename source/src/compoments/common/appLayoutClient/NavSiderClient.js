...
class NavSiderClient extends Component {
    constructor(props) {
        super(props)
        this.state = {
    ...
    render() {
        const {currentPathname, userData, t } = this.props;
        const {
            loadingMenuItem,
        } = this.state;
        let menuConfig = []
        ...
        if(userData?.kind === UserTypes.TEACHER){
            menuConfig= teacherNavMenuConfig  
        }
        if(userData?.kind === UserTypes.STUDENT){
            menuConfig= studentNavMenuConfig  
        }
...
export default withTranslation('navSider')(NavSiderClient);
